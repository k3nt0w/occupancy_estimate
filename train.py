...
for epoch in range(1, n_epoch+1):
    ...
    for i in range(n_iter):
        ...
        y = xp.zeros((batchsize, n_class), dtype=xp.float32)
        for j in range(batchsize):
            name = names[i*batchsize + j]
            xpath = train_dataset+name+".jpg"
            ypath = target_dataset+name+".png"
            x[j] = load_data(xpath, crop=True, size=image_size, mode="data", xp=xp)
            y[j] = load_data(ypath, crop=True, size=image_size, mode="label", xp=xp)
        x = Variable(x)
        y = Variable(y)
        loss = model(x, y)
...
if not os.path.exists("weight"):
    os.mkdir("weight")
serializers.save_npz('weight/weight', model)
serializers.save_npz('weight/state', optimizer)
print('save weight')
